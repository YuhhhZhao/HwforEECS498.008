...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(dists.shape[0]):
        for j in range(dists.shape[1]):
            dists[i,j] = torch.sum((x_train[i].clone() - x_test[j].clone())**2)
    ################## ########################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i] = torch.sum((x_test_flat - x_train_flat[i]).pow(2), dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_reshaped = x_test.reshape(num_test,-1)
    x_train_reshaped = x_train.reshape(num_train,-1)
    sum1 = torch.sum(x_train_reshaped.pow(2), dim = 1, keepdim= True)
    sum2 = torch.sum(x_test_reshaped.pow(2), dim = 1)
    mulp = x_train_reshaped @ x_test_reshaped.t()
    dists = sum1 + sum2 - 2 * mulp
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values, indices = torch.topk(dists, k, largest = False, dim = 0)
    for i in range(num_test):
        k_nearest_labels = y_train[indices[:, i]]
        dicts = {}
        for j in k_nearest_labels:
            dicts[j.item()] = dicts.get(j.item(),0) + 1
        max_value = max(dicts.values())
        y_pred[i] = 10000000
        for k in dicts:
            if max_value == dicts[k]:
                y_pred[i] = min(y_pred[i], k)
...
def knn_cross_validate(
    ...
    for k_c in k_choices:
        ...
        for i in range(num_folds):
            x_train_folds_i = torch.tensor([])
            y_train_folds_i = torch.tensor([])
            for j in range(num_folds):
                if j != i:
                    x_train_folds_i = torch.cat((x_train_folds_i,x_train_folds[j]))
                    y_train_folds_i = torch.cat((y_train_folds_i,y_train_folds[j]))
            K_cnn = KnnClassifier(x_train_folds_i,y_train_folds_i)
            acc_k_c.append(K_cnn.check_accuracy(x_train_folds[i],y_train_folds[i],k = k_c))
        k_to_accuracies[k_c] = acc_k_c
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    max_acc = 0
    for k_c in k_to_accuracies:
        if max_acc < sum(k_to_accuracies[k_c]) / len(k_to_accuracies[k_c]):
            max_acc = sum(k_to_accuracies[k_c]) / len(k_to_accuracies[k_c])
            best_k = k_c
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
