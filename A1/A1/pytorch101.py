...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    idx = torch.arange(y.shape[0],dtype = torch.int64)
    values = torch.ones(len(x),dtype = torch.float32)
    y[idx, x] = values
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    col_min_idxs1 = x.argmin(dim = 1)
    col_min_idxs0 = torch.arange(x.shape[0])
    print('\n',col_min_idxs0,col_min_idxs1)
    y = x.clone()
    y[col_min_idxs0,col_min_idxs1] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_sum = torch.sum(x.clone(), 0)
    x_av = x_sum / x.shape[0]
    x_axis = x.clone() - x_av
    x_axis = x_axis**2
    x_axis_sum = torch.sum(x_axis, 0)
    x_axis_sum = x_axis_sum / (x.shape[0]-1)
    sigma_j = torch.sqrt(x_axis_sum)   
    y = (x - x_av) / sigma_j
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
