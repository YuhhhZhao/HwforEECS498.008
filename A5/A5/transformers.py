...
def prepocess_input_sequence(
    ...
    for token in input_str.split():
        if token in spc_tokens:
            # If the token is a special token, assign its value from the dictionary
            out.append(token_dict[token])
        else:
            # If the token is a number, break it down into digits and assign values
            for digit in token:
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = torch.zeros(K, M).to(query.device)
    for i in range(K):
        # Compute the dot product of the i-th query with all keys
        dot_product = torch.zeros(K).to(query.device)
        for j in range(K):
            dot_product[j] = torch.inner(query[i], key[j])
        # Scale the dot product by sqrt(M)
        scaled_dot_product = dot_product / (M**0.5)
        # Apply softmax to get weights
        weights = F.softmax(scaled_dot_product, dim=0)
        # Compute the weighted sum of values
        out[i] = torch.sum(weights.view(1,-1).mm(value), dim=0)
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros(N, K, M).to(query.device)
    for i in range(N):
        # Compute the dot product of the i-th query with all keys
        dot_product = torch.zeros(K, K).to(query.device)  
        dot_product = query[i].mm(key[i].T)
        # Scale the dot product by sqrt(M)
        scaled_dot_product = dot_product / (M**0.5)
        # Apply softmax to get weights
        weights = F.softmax(scaled_dot_product, dim = 1)
        # Compute the weighted sum of values
        out[i] = weights.mm(value[i])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights_softmax = query.bmm(key.transpose(1, 2)) / (M**0.5)
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(weights_softmax, dim=2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # and weights to a uniform distributions
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.linears = nn.Linear(dim_out * num_heads, dim_in)
        cd = (6/(num_heads*dim_out + dim_in))**0.5
        torch.nn.init.uniform_(self.linears.weight, a=-cd, b=cd)
        torch.nn.init.constant_(self.linears.bias, 0)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        attention_outputs = [   
            attention(query, key, value, mask) for attention in self.MultiAttention
        ]
        y = torch.cat(attention_outputs, dim=-1)
        y = self.linears(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        cd = (6/(inp_dim + hidden_dim_feedforward))**0.5
        torch.nn.init.uniform_(self.mlp1.weight, a=-cd, b=cd)
        torch.nn.init.constant_(self.mlp1.bias, 0)
        cd = (6/(hidden_dim_feedforward + inp_dim))**0.5
        torch.nn.init.uniform_(self.mlp2.weight, a=-cd, b=cd)
        torch.nn.init.constant_(self.mlp2.bias, 0)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.zeros(N, K, K).to(seq.device, dtype=torch.bool)
    for i in range(N):
        for j in range(K):
            mask[i, j, :j+1] = False
            mask[i, j, j+1:] = True
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out6 = self.dropout(self.norm3(out4 + out5))
        y = out6
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.arange(K).reshape(1, K, 1).to(torch.float32) / K
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    def __init__(
        ...
        cd = (6 / (vocab_len + emb_dim)) ** 0.5
        torch.nn.init.uniform_(self.emb_layer.weight, -cd, cd)
        torch.nn.init.constant_(self.emb_layer.weight, 0)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        dec_out = decoder_out.reshape(decoder_out.shape[0]*decoder_out.shape[1],-1 )
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
