...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    X1 = torch.mm(X, W1) + b1
    X1_bool = X1 > 0
    hidden = X1 * X1_bool.to(torch.int64)
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores_max,_ = torch.max(scores, 1)
    exp_scores = torch.exp(scores - scores_max.view(-1,1))
    softmax_scores = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True)
    correct_class_score = softmax_scores[torch.arange(N), y]
    loss = -torch.sum(torch.log(correct_class_score))
    loss /= N
    ...
    dscore /= N
    dex = torch.ones(1, N).to(X)
    grads['b2'] = dex.mm(dscore).view(-1)
    grads['W2'] = h1.t().mm(dscore) + 2 * reg * W2
    dy2dy1 = torch.mm(dscore, W2.t())
    dy2dy1 = dy2dy1 * (h1 > 0)
    grads['b1'] = dex.mm(dy2dy1).view(-1)
    grads['W1'] = X.t().mm(dy2dy1) + 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    Y = torch.maximum(X.mm(params['W1']) + params['b1'], torch.zeros(X.shape[0], params["W1"].shape[1]).to(X)).mm(params['W2']) + params['b2']
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.47,1.475]
    hidden_sizes = [512]
    regularization_strengths = [26e-5, 27e-5]
    ...
    for lr in learning_rates:
        for hs in hidden_sizes:
            ...
            reg=rs, verbose=False)
                    ...
                    stat_val_acc = 100.0 * (y_val_pred == data_dict['y_val']).double().mean().item()
                    print('lr %e hs %e rs %e lrd %e val accuracy: %f' % (lr, hs, rs, lrd, stat_val_acc))
                    if stat_val_acc > best_val_acc:
                        best_net = net
                        best_stat = stats
                        best_val_acc = stat_val_acc
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
