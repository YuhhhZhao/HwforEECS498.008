...
def svm_loss_vectorized(
    ...
    margin_bool = margin_bool.to(X)
    margin_sum = torch.sum(margin_bool, dim = 1)
    margin_bool[torch.arange(num_train), y] = -margin_sum
    dW = X.t().mm(margin_bool)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    value, y_pred = torch.max(X.mm(W), 1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-2,5e-2,1e-3,5e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        correct_class_score = scores[y[i]]
        margin = -torch.log(correct_class_score / torch.sum(scores)) 
        loss += margin
        for j in range(num_classes):
            dW[:, j] += (scores[j]/torch.sum(scores) - (j == y[i]).to(torch.int64)) * X[i]
    loss /= num_train
    loss += reg * torch.sum(W * W)
    dW /= num_train
    dW += 2 * reg * W
...
def softmax_loss_vectorized(
    ...
    scores -= scores_max.view(-1, 1)
    exp_scores = torch.exp(scores)
    softmax_scores = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True)
    correct_class_score = softmax_scores[torch.arange(num_train), y]
    scores = torch.exp(scores)
    scores_log = torch.log(correct_class_score)
    loss += -1 * torch.sum(scores_log)
    loss /= num_train
    loss += reg * torch.sum(W * W)

    y_label = torch.zeros((num_train,num_classes)).cuda()
    y_label[torch.arange(num_train), y] = 1
    dW = X.t().mm(softmax_scores - y_label)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2, 1e-1, 5e-3, 5e-2, 5e-1]
