          ...
          for f in range(F):
              for h_out in range(H_out):
                  for w_out in range(W_out):
                      ...
                      out[n, f, h_out, w_out] = torch.sum(x_slice * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for c in range(C):
                for h in range(HH):
                    for w_i in range(WW):
                        ...
                        for h_out in range(H_out):
                            for w_out in range(W_out):
                                value += dout[n, f, h_out, w_out] * x_pad[n, c, h_out * stride + h, w_out * stride + w_i]
                        dw[f, c, h, w_i] += value
                ...
                for c in range(C):
                    for h in range(H):
                       for w_i in range(W):
                           ...
                           for h_out in range(H_out):
                               for w_out in range(W_out):
                                  h_filter = h + pad - h_out * stride
                                  w_filter = w_i + pad - w_out * stride
                                  if 0 <= h_filter < HH and 0 <= w_filter < WW:
                                    v += dout[n, f, h_out, w_out] * w[f, c, h_filter, w_filter]
                           dx[n, c, h, w_i] += v
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(N, C, 1 + (H - pool_height) // stride, 1 + (W - pool_width) // stride).to(x)
        for n_i in range(N):
            for c_i in range(C):
                for h_i in range(out.shape[2]):
                    for w_i in range(out.shape[3]):
                        h_is = h_i * stride
                        w_is = w_i * stride
                        x_sliced = x[n_i, c_i, h_is:h_is + pool_height, w_is:w_is + pool_width]
                        out[n_i, c_i, h_i, w_i] = torch.max(x_sliced)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
            ...
            for c_i in range(C):
                for h_i in range(HH):
                    for w_i in range(WW):
                        ...
                        w_is = w_i * stride
                        x_sliced = x[n_i, c_i, h_is:h_is + pool_height, w_is:w_is + pool_width]
                        index = torch.argmax(x_sliced)
                        row = index // pool_height
                        col = index % pool_height
                        dx[n_i, c_i, h_is + row, w_is + col] = dout[n_i, c_i, h_i, w_i]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = torch.normal(0, weight_scale, size=(fc_input_dim, hidden_dim),dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        out_linear_input = out.reshape(out.shape[0],-1)
        out_hidden_input, cache_linear = Linear_ReLU.forward(out_linear_input, W2, b2)
        scores,cache_out = Linear_ReLU.forward(out_hidden_input, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        loss += self.reg * (torch.sum(W3 * W3) + torch.sum(W2 * W2) + torch.sum(W1 * W1))
        dx1, grads['W3'], grads['b3'] = Linear_ReLU.backward(dout, cache_out)
        grads['W3'] += 2 * self.reg * W3
        dx2, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx1, cache_linear)
        grads['W2'] += 2 * self.reg * W2
        dx_conv = dx2.reshape(dx2.shape[0], W1.shape[0], X.shape[2] // 2, X.shape[3] // 2)
        _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx_conv, cache_conv)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        C, H, W = input_dims
        curr_H, curr_W = H, W
        num_filters_input = [C] + num_filters
        for i in range(self.num_layers-1):
          w_choose = 'W' + str(i)
          b_choose = 'b' + str(i)
          beta_choose = 'beta' + str(i)
          gamma_choose = 'gamma' + str(i)
          if weight_scale == 'kaiming':
              self.params[w_choose] = kaiming_initializer(num_filters_input[i], num_filters[i], 3, True, device, dtype)
          else:
              self.params[w_choose] = torch.normal(0, weight_scale, size=(num_filters[i], num_filters_input[i], 3, 3),dtype=dtype, device=device)
          self.params[b_choose] = torch.zeros(num_filters[i],dtype=dtype, device=device)
          if self.batchnorm:
            self.params[beta_choose] = torch.zeros(num_filters[i], dtype=dtype, device=device)
            self.params[gamma_choose] = torch.ones(num_filters[i], dtype=dtype, device=device)
          if i in max_pools:
              curr_H //= 2
              curr_W //= 2
        self.fulllayer_size = num_filters_input[-1] * curr_H * curr_W
        if weight_scale == 'kaiming':
            self.params['W' + str(self.num_layers-1)] = kaiming_initializer(self.fulllayer_size, num_classes,None, False, device, dtype)
        else:
            self.params['W' + str(self.num_layers-1)] = torch.normal(0, weight_scale, size=(self.fulllayer_size, num_classes),dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        if not self.batchnorm:
          for i in range(self.num_layers-1):
              ...
              if i in self.max_pools:
                  out, cache[i] = Conv_ReLU_Pool.forward(out, self.params[w_choose], self.params[b_choose], conv_param, pool_param)
              else:
                  out, cache[i] = Conv_ReLU.forward(out, self.params[w_choose], self.params[b_choose], conv_param)
          out = out.reshape(out.shape[0], -1) 
          scores, cache_linear = Linear.forward(out, self.params['W' + str(self.num_layers-1)], self.params['b' + str(self.num_layers-1)])
        else:
          for i in range(self.num_layers-1):
              w_choose = 'W' + str(i)
              b_choose = 'b' + str(i) 
              ...
              gamma_choose = 'gamma' + str(i)
              if i in self.max_pools:
                out, cache[i] = Conv_BatchNorm_ReLU_Pool.forward(out, self.params[w_choose], self.params[b_choose], self.params[gamma_choose], self.params[beta_choose],conv_param, self.bn_params[i], pool_param)
              else:
                out, cache[i] = Conv_BatchNorm_ReLU.forward(out, self.params[w_choose], self.params[b_choose], self.params[gamma_choose], self.params[beta_choose],conv_param, self.bn_params[i])
          out = out.reshape(out.shape[0], -1) 
          scores, cache_linear = Linear.forward(out, self.params['W' + str(self.num_layers-1)], self.params['b' + str(self.num_layers-1)])
        #####################################################
        #                 END OF YOUR CODE                  #
        ...
        # Replace "pass" statement with your code
        loss, dout = softmax_loss(scores, y)
        dout, grads['W' + str(self.num_layers-1)], grads['b' + str(self.num_layers-1)] = Linear.backward(dout, cache_linear)
        grads['W' + str(self.num_layers-1)] += 2 * self.reg * self.params['W' + str(self.num_layers-1)]
        loss += self.reg * torch.sum(self.params['W' + str(self.num_layers-1)]**2)
        dout = dout.reshape(X.shape[0], self.params['W' + str(self.num_layers-2)].shape[0], X.shape[2]//(2 ** len(self.max_pools)),X.shape[3]//(2 ** len(self.max_pools)))
        for i in range(self.num_layers - 2 , -1 , -1):
            if self.batchnorm:
                if i in self.max_pools:
                  dout, grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] = Conv_BatchNorm_ReLU_Pool.backward(dout, cache[i])
                else:
                  dout, grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] = Conv_BatchNorm_ReLU.backward(dout, cache[i])
                grads['W' + str(i)] += 2 * self.reg * self.params['W' + str(i)]
                loss += self.reg * torch.sum(self.params['W' + str(i)]**2)
            else:
                if i in self.max_pools:
                  dout, grads['W' + str(i)], grads['b' + str(i)] = Conv_ReLU_Pool.backward(dout, cache[i])
                else:
                  dout, grads['W' + str(i)], grads['b' + str(i)] = Conv_ReLU.backward(dout, cache[i])
                grads['W' + str(i)] += 2 * self.reg * self.params['W' + str(i)]
                loss += self.reg * torch.sum(self.params['W' + str(i)]**2)
        #############################################################
        #                       END OF YOUR CODE                    #
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_filters=[32, 256], max_pools=[0,1],batchnorm=False, num_classes=10,
                         weight_scale='kaiming', reg = 5e-5, dtype=dtype, device = device)
    solver = Solver(model, small_data, num_epochs=40, batch_size=128,
                  update_rule=adam,
                  optim_config={
                    'learning_rate': 25e-5,
                    'epsilon':1e-4,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(0, (gain / Din)**0.5,size=(Din , Dout), device = device, dtype = dtype)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(0, (gain / Din)**0.5,size=(Dout , Din , K , K), device = device, dtype = dtype)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            running_mean =  momentum * running_mean + (1 - momentum) * sample_mean
            running_var =  momentum * running_var + (1 - momentum) * sample_var
            cache = (x, sample_mean, sample_var, x_norm, gamma, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            u_x = running_mean
            var_x = running_var
            out = gamma * (x - u_x) / (var_x + eps)**0.5 + beta
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x, sample_mean, sample_var, x_norm, gamma, eps = cache
        N, D = x.shape
        dgamma = torch.sum(dout * x_norm, dim = 0)
        dbeta = torch.sum(dout, dim = 0)
        dxnorm = gamma * dout
        dvar = torch.sum(dxnorm * (x - sample_mean) * (-0.5) * (sample_var + eps)**(-1.5), dim = 0)
        du = torch.sum(dxnorm * (-1) * (sample_var + eps)**(-0.5), dim=0) + \
         dvar * torch.sum(-2 * (x - sample_mean), dim=0) / N
        dx = dxnorm * (sample_var + eps)**(-0.5) + dvar * 2 * (x - sample_mean) / N + du / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dbeta = torch.sum(dout, dim=0)
        dx_norm = dout * gamma
        dvar = torch.sum(dx_norm * (x - mean) * (-0.5) * (var + eps)**(-1.5), dim=0)
        dmean = torch.sum(dx_norm * (-1) / torch.sqrt(var + eps), dim=0) + dvar * torch.sum(-2 * (x - mean), dim=0) / N
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        out_bn, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)
        out = out_bn.reshape(N, H, W, C).permute(0, 3, 1, 2) 
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
