...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        X2, relu_cache = Linear_ReLU.forward(X, W1, b1)
        scores,relu_cache2 = Linear.forward(X2, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dx2, dw2, db2 = Linear.backward(softmax_grad, relu_cache2)
        dx, dw1, db1 = Linear_ReLU.backward(dx2, relu_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers - 1):
            W_string = 'W' + str(i+1)
            b_string = 'b' + str(i+1)
            self.params[W_string] = torch.normal(0, weight_scale, size=(hidden_dims[i-1], hidden_dims[i])).to(dtype).to(device)
            self.params[b_string] = torch.zeros(hidden_dims[i]).to(dtype).to(device)
        self.params['W' + str(self.num_layers)] = torch.normal(0, weight_scale, size=(hidden_dims[len(hidden_dims)-1], num_classes)).to(dtype).to(device)
        self.params['b' + str(self.num_layers)] = torch.zeros(num_classes).to(dtype).to(device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            W_string = 'W' + str(i+1)
            b_string = 'b' + str(i+1)
            out, ReLU_cache[i+1] = Linear_ReLU.forward(out, self.params[W_string], self.params[b_string])
            if self.use_dropout:
                out, Dropout_cache[i+1] = Dropout.forward(out,self.dropout_param)
        scores, ReLU_cache[self.num_layers] = Linear.forward(out, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        loss, softmax_grad = softmax_loss(scores, y)
        softmax_grad, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = Linear.backward(softmax_grad, ReLU_cache[self.num_layers])
        grads['W' + str(self.num_layers)] += 2 * self.reg * self.params['W' + str(self.num_layers)]
        loss += self.reg * torch.sum(self.params['W' + str(self.num_layers)] * self.params['W' + str(self.num_layers)])
        for i in range(self.num_layers-2, -1, -1):
            W_string = 'W' + str(i+1)
            b_string = 'b' + str(i+1)
            Wi = self.params[W_string]
            bi = self.params[b_string]
            loss += self.reg * torch.sum(Wi * Wi)
            if self.use_dropout:
                softmax_grad = Dropout.backward(softmax_grad, Dropout_cache[i+1])
            softmax_grad, grads[W_string], grads[b_string] = Linear_ReLU.backward(softmax_grad, ReLU_cache[i+1])
            grads[W_string] += 2 * self.reg * Wi
...
def rmsprop(w, dw, config=None):
    ...
    config['learning_rate'] /= (config['cache'] + config['epsilon'])**(0.5)
...
def adam(w, dw, config=None):
    ...
    config['t'] += 1
    m_modify = config['m'] / (1 - config['beta1'] ** config['t'])
    v_modify = config['v'] / (1 - config['beta2'] ** config['t'])
    next_w = w - config['learning_rate'] / (v_modify**0.5 + config['epsilon']) * m_modify
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand_like(x).to(x.device).to(x.dtype) < (1 - p)) / (1 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
