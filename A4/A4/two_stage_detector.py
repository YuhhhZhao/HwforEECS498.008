...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            if i == 0:
                conv = nn.Conv2d(in_channels, stem_channels[i], 3, padding= 'same')
            else:
                conv = nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, padding= 'same')
            mean = 0
            std = 0.01
        ...
        self.pred_obj = nn.Conv2d(
            stem_channels[-1], num_anchors, 1, padding= 'same')
        self.pred_box = nn.Conv2d(
            stem_channels[-1], num_anchors * 4, 1, padding= 'same') 
        mean = 0
        std = 0.01
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, fpn_feats in feats_per_fpn_level.items():
            object_logits[level_name] = self.pred_obj(self.stem_rpn(fpn_feats)).flatten(1)
            boxreg_deltas[level_name] = self.pred_box(self.stem_rpn(fpn_feats))
            batch_size, _, H, W = boxreg_deltas[level_name].shape
            boxreg_deltas[level_name] = boxreg_deltas[level_name].reshape(batch_size, 4, -1, H, W).flatten(2).transpose(1, 2)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    area1 = ((boxes1[:,2]-boxes1[:,0])*(boxes1[:,3]-boxes1[:,1])) # shape (M,)
    area2 = ((boxes2[:,2]-boxes2[:,0])*(boxes2[:,3]-boxes2[:,1])) # shape (N,)
    ###
    # print(boxes1.device, boxes2.device)
    ###
    x1 = torch.maximum(boxes1[:, 0].unsqueeze(1), boxes2[:, 0]) # shape (M, N)
    ...
    invalid_idx = torch.logical_or(x1 > x2, y1 > y2) # shape (M, N)
    inter_boxes = torch.stack((x1, y1, x2, y2), dim=-1) # shape (M, N, 4)
    ibr = inter_boxes.reshape(-1, 4)
    inter_areas = ((ibr[:,2]-ibr[:,0])*(ibr[:,3]-ibr[:,1])).reshape(M, N)
    union_areas = area1.unsqueeze(-1) + area2 - inter_areas # shape (M, N)
...
def rcnn_get_deltas_from_anchors(
    ...
    idx = gt_boxes[:, 0] > -1 # non-background gt boxes indices
    a = anchors[idx]
    gt = gt_boxes[idx]
    px = (a[:, 0] + a[:, 2]) / 2
    py = (a[:, 1] + a[:, 3]) / 2
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = None
    # Replace "pass" statement with your code
    idx = deltas == -1e8 # non-background gt boxes indices
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        for _idx in range(num_images):
            ...
            _gt_boxes = gt_boxes[_idx, :, :]

            # Get matched GT boxes for this image:
            _matched_gt_boxes = rcnn_match_anchors_to_gt(
                anchor_boxes, _gt_boxes, self.anchor_iou_thresholds
            )

            # Append to list of matched GT boxes:
            matched_gt_boxes.append(_matched_gt_boxes)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
