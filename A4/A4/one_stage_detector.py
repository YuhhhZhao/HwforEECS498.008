...
def fcos_get_deltas_from_locations(
    ...
    x0, y0, x1, y1, _ = gt_boxes.unbind(dim=1)
    deltas = torch.stack([x-x0, y-y0, x1-x, y1-y], dim=1).to(locations.device)
    deltas = deltas / stride    # Normalize by stride
    
    # 更高效的方式处理背景框
    is_bg = (x0 == -1) & (y0 == -1) & (x1 == -1) & (y1 == -1)
    deltas[is_bg] = -1
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes = torch.stack([x0, y0, x1, y1], dim=1).to(deltas.device)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for i in range(images.shape[0]):
            # 确保所有输入都在同一设备上，将locations_per_fpn_level移到和gt_boxes相同的设备
            locations_for_matching = {}
            for level_name, locations in locations_per_fpn_level.items():
                locations_for_matching[level_name] = locations.to(gt_boxes.device)
            
            matched_gt_boxes.append(
                fcos_match_locations_to_gt(
                    locations_for_matching,  # 使用在正确设备上的locations
        ...
        for i in range(images.shape[0]):
            ...
            for level_name in locations_per_fpn_level.keys():
                # 确保 locations 和 gt_boxes 在同一设备上
                locations = locations_per_fpn_level[level_name].to(gt_boxes.device)
                gt_boxes_level = matched_gt_boxes[i][level_name].to(gt_boxes.device)
                
                matched_gt_deltas[i][level_name] = fcos_get_deltas_from_locations(
                    locations, 
                    gt_boxes_level,
        ...
        gt_classes = matched_gt_boxes[:, :, -1].to(dtype=torch.long)
        bg_idx = gt_classes == -1
        gt_classes[bg_idx] = self.num_classes
        # shape (batch_size, sum_resolution, num_classes)
        gt_classes = F.one_hot(gt_classes, num_classes=self.num_classes+1)[:,:,:-1].float()
        ...
        # 确保在计算centerness loss前，所有张量都在同一设备上
        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas.reshape(-1, 4)).to(device=pred_ctr_logits.device)
        
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.view(-1), gt_centerness.view(-1), reduction="none"
        )
        # No loss for background:
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # 确保所有张量在同一设备上
            device = level_cls_logits.device
            level_locations = level_locations.to(device)
            ...
            # Step 1: 获取每个框最有信心的预测类别及其分数
            pred_scores, level_pred_classes = torch.max(level_pred_scores, dim=1)

            # Step 2: 只保留置信度分数高于阈值的预测
            conf_idx = pred_scores > test_score_thresh
            level_pred_classes = level_pred_classes[conf_idx]
            level_pred_scores = pred_scores[conf_idx]

            # Step 3: 根据预测的deltas和位置获取预测框
            stride = self.backbone.fpn_strides[level_name]
            level_deltas = level_deltas[conf_idx]
            level_locations = level_locations[conf_idx]  # 现在level_locations和conf_idx在同一设备上
            ...
            level_pred_boxes[:, 2] = level_pred_boxes[:, 2].clamp(max=width-1)
            level_pred_boxes[:, 3] = level_pred_boxes[:, 3].clamp(max=height-1)
        ...
        # 确保所有列表非空再进行拼接
        if len(pred_boxes_all_levels) == 0 or all(len(x) == 0 for x in pred_boxes_all_levels):
            return torch.zeros(0, 4, device=device), torch.zeros(0, dtype=torch.long, device=device), torch.zeros(0, device=device)
